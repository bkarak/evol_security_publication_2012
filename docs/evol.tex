...
and moreover it can cause significant financial damage to 
an organization~\cite{TH04}\cite{BCL08}\cite{R06}. 
Specifically, a software bug can
...
%@dimitro: check the following
The main contributions of this work are:
\begin{itemize}
	\item We present evidence that defect counts remain relatively stable
    across project revisions.
%An analysis of how security bugs found through static analysis,
%evolve over time.
%To achieve this, we inspect every project per version.
...
%they are critical and developers should eliminate them.
	\item We find that a specific defect remains unfixed on average 
    for 3 revisions.
% Security bug persistence between versions. 
%We expect that
%security bugs should be eliminated earlier than other bugs.
	\item We show that many defect types are correlated with the project's
    size (with the notable exception of high-risk security defects).
%Our hypothesis is that security bugs are proportional to a project
%version's size.  Here, size is defined in terms of bytecode.
	\item We show that a code base can have defect hotspots.
%Our
...
%should have a low rate of security bugs. 
  \item We make available a carefully constructed dataset and a statistical toolkit for
