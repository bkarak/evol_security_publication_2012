...
research~\cite{AP10}\cite{HP07}\cite{HP04}\cite{HW08}. Specifically, we ran FindBugs on all the project
...
fits on the data for reasons that we will describe below.
...
maven-metadata.xml}. Then, FindBugs was executed and its results were also stored
in the data repository. When the task was completed the queue was notified and
the next task was requested.
...
Malicious Code}. From the total number of the {\sc jar}s, 4,353 of them contained
...
% in a recent paper, Spacco et al.~\cite{SHP06} showed the relation of bugs
% in general, with the size of every version of Sun's {\sc jdk}.
