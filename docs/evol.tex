...
Section~\ref{sec:rel}). Furthermore, consider the case where a
security bug is reported, but it is invalidated based on the context of the
application. For instance, an {\sc sql} injection vulnerability~\cite{RL12}
in an application that requires no external input could be reported by FindBugs,
but in this case would be a false positive alarm.
