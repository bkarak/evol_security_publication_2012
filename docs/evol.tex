...
security bugs and project evolution.
In addition, they are not eliminated in a way that is particularly
different from the other bugs. Their relation with
size appears to be different from the other categories. Finally, even if bugs
...
evolve over time. To achieve this, we inspected every project per version.
...
behavior of security bugs. Specifically, we analyzed more than 260GB
...
ones, should be corrected as projects evolve. We found that, although
...
increase, as well as decrease in time. Contrary to our second research hypothesis, we showed that security
