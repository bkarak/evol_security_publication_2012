...
We used the Maven repository to study the
...
indicated that security bugs neither increase or decrease as a project evolves.
In addition, they are not eliminated in a way that is particularly
different from the other bugs. Also, their relation with the
size appeared to be different from the other categories. Finally, even if bugs
seem to flock together, severe security bugs
seem to be unassociated with other categories.
\end{abstract}
...
how different types of security vulnerabilities are related to the evolving software packages.
...
and it costs~\cite{BCL08}\c~\cite{TH04}ite{R06}. Specifically, a software bug can
...
<<<<<<< HEAD
done) but also with other bug categories. In the same manner, we tried to check
the relation between version size and security bugs, knowing the
=======
...
>>>>>>> changes after Panos and dds comments
...
\begin{itemize}
	\item The analysis of how security bugs found through static analysis,
evolve over time. To achieve this, we inspect every project per version.
Our hypothesis is that security bugs should decrease as a project evolves since
they are critical and developers should eliminate them.
	\item Security bug persistence between versions. We expect that
security bugs should be eliminated earlier than other bugs.
	\item The relation between security bugs and a project
version's size. Our hypothesis is that security bugs are proportional to a project
version's size.  Here, size is defined in terms of bytecode.
	\item The correlation of security bugs with other bug categories. Our
hypothesis is that security bugs flock together with bugs that have to do with
...
for all the project versions existing on the Maven repository.
...
FindBugs tool. The false positives and negatives of static analysis tools and
how they can be reduced, is an issue that has already been discussed in literature (see
Section~\ref{sec:rel}). Furthermore, consider a false positive alarm where an
security bug is reported, but it is invalidated based on the context of the
application (an {\sc sql} injection vulnerability~\cite{RL12} in an application that
requires no external input).

We also have to mention that our analysis is done on projects written in the
Java programming language. Hence, a threat to the external validity of our work
is the fact that our results may not be the same in another programming
environment.
...
coding practice. \\
Correctness & Involves coding mistin a way that is particularly different from the other bugsakes resulting in code
...
To examine {\it Linus' Law} in relation to the rate of the
security bugs~\cite{MW10}, and highlight the
{\it domino effect}~\cite{TH04} we did the
following: during the experiment we managed to retrieve the
...
Contrary to Linus' Law, thirty three of them contained bugs coming from the
...
\subsubsection{How Security Bugs E(over versions)volve Over Time}
...
Figure~\ref{fig:bugsversionscorr} suggest that we cannot say that
...
Figure~\ref{fig:bugsversionscorr} suggests that \textbf{we cannot say that across
...
\ll 0.05$) can be seen in Table~\ref{tbl:jarsizecorr}. The {\it
...
0.05$) can be seen in Table~\ref{tbl:jarsizecorr}. \textbf{The {\it Security
...
ones, should be corrected as projects evolve. However, our findigs indicate that this is
not true. Contrary to our second research hypothesis, we showed that security
...
we showed evidence that they are not proportionally related. Furthermore, the
pairwise correlations between all categories indicated that even though all the other
categories are related, severe bugs do not flock together with the other bugs.
...
domino effect, and showed evidence that indicates that the Linus' Law does not
apply in the case of the security bugs.

Contrary to the approaches that examine all versions
formed after every change that has been commited to the repository,
our observations are made from a different perspective.
The versions examined in this work were actual releases
of the projets. As a result we do not have an indication of how many changes
