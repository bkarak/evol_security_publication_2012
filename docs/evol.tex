...
Figure~\ref{fig:bugsversioncorr}. The spike in point zero is explained
by the large number of projects for which no correlation could be
established---note that the scale is logarithmic. 

But we can see that there were projects where a correlation could be
...
together with Table~\ref{tbl:bugsperversion} this suggests that: we
cannot say that across projects defect counts increase or decrease
significantly across time. But in individual projects, defect counts
can have a strong upwards or downwards tendency. There may be no such
thing as a ``project'' in general, only particular projects with their
own idiosyncrasies, quality features, and coding practices.

\begin{figure*}
  \centering
  \includegraphics[scale=0.4]{bugsversionscorr.pdf}
  \caption{Histograms of correlations between bug counts and version
    ordinals per project}
