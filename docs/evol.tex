...
lifetime. The resulting correlation plot can be seen in
Figure~\ref{fig:corrplot}. We found significant correlations between
all pairs apart from those involving the {\it Security Low} category,
due to the small number of bugs in that category (19). The results
show then that bugs do flock together: we do not find projects with
only a certain kind of bug; bugs come upon projects in swarms of
different kinds. Since the correlations with the {\it Security High}
category are particularly high, the presence of other bugs should be a
strong indicator to have a better look at security aspects of code.
\begin{table}
